			...
			if (j.mtime !== mtime){
				...
				return scrape(urlToScrape).then(function(popular){
					...
					j.stories = j.stories.map(function(item){
						...
						// Determine the image to use as a thumbnail
						var pos,
							posVariant
						pos = item.images.findIndex(function(image){
						...
						item.thumbnail = (pos === -1) ? '': item.images[pos].variants[posVariant].src;

						// Change the URL to the HTML version
